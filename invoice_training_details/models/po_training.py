...
class PurchaseOrder(models.Model):
    ...
    training_course_ids = fields.One2many(
        'training.course', 
        'purchase_order_id',  # Changed from 'order_id' to 'purchase_order_id'
        string='Training Courses'
    )
    pro_service_ids = fields.One2many(
        'pro.service',
        'purchase_order_id',  # Changed from 'pro_sale_id' to 'purchase_order_id'
    ...
    ticket_ids = fields.One2many(
        'ticket.ticket',
        'purchase_order_id',  # Changed from 'ticket_order_id'
        string='Tickets'
    )
    hotel_ids = fields.One2many(
        'hotel.hotel',
        'purchase_order_id',  # Changed from 'hotel_order_id'
        string='Hotels'
    )
    ...
    end_date = fields.Date(string="To Date")  # Changed from 'to_date' to 'end_date'
    
    # Attachments
    book_details_id = fields.Many2many(
        'ir.attachment', 
        'doc_attach_purchase_order',  # Changed relation table name
        'doc_id', 
        'attach_order_id',
    ...
    po_reference = fields.Char(string='PO#')  # Changed from 'so_no' to 'po_reference'
    tr_expiry_date = fields.Date(string='Expiry Date')
    
    # Terms and Conditions
    bank_details = fields.Html(
        string='Bank Details',
        default='We kindly request you to transfer OR deposit cheque payment to below bank account details </br> Account Name: NIL Data Communications Middle East DMCC Emirates Islamic Bank JLT Branch - Dubai- UAE </br> Swiftcode: MEBLAEAD </br> Account Currency: USD </br> IBAN: AE690340003528215597102'
    )
    ...
    def action_sync_training_lines(self):
        """Sync training courses to purchase order lines"""
        self.ensure_one()
        order_lines = []
        for course in self.training_course_ids:
            order_lines.append((0, 0, {
                ...
                'date_planned': self.date_order or fields.Date.today(),
            }))
    ...
    def action_sync_service_lines(self):
        """Sync professional services to purchase order lines"""
        self.ensure_one()
        order_lines = []
