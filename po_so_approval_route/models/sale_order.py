...
class SaleOrder(models.Model):
    ...
    so_order_approval_route = fields.Selection(related='company_id.so_order_approval_route',
    ...
    def button_approve(self, force=False):
        for order in self:
            if not order.team_custom_id:
                ...
                super(SaleOrder, order).button_approve(force)
                ...
                if order.current_approver.user_id == self.env.user or self.env.is_superuser():
                    ...
                    order.message_post(body=_('SO approved by %s') % self.env.user.name)
    ...
    def send_to_approve(self):
        for order in self:
            ...
            if not order.next_approver:
                reason_msg = _("There are no approvers in the selected SO team.")
                raise UserError("%s %s" % (main_error_msg, reason_msg))
    ...
    def _check_lock_amount_total(self):
        ...
        for order in self:
            ...
            if order.team_custom_id.lock_amount_total:
                ...
                suggestion = _('To make changes, cancel and reset SO to draft. ')
                raise UserError(msg + "\n\n" + reason + "\n\n" + suggestion)