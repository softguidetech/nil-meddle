...
class SaleTeam(models.Model):
    _name = "sale.team.custom"
    _description = "SO Team"
    ...
    approver_ids = fields.One2many('sale.team.approver','team_custom_id', string='Approvers')

    @api.constrains('company_id')
    def _check_team_company(self):
        for team in self:
            if team.company_id.so_order_approval_route == 'no':
...
class SaleTeamApprover(models.Model):
    ...
    _description = "SO Team Approver"
    _order = 'sequence'
    _rec_name = 'sequence'

    sequence = fields.Integer(string='Sequence', default=10)

    team_custom_id = fields.Many2one('sale.team.custom', string='Team', ondelete='cascade')
    ...
    lock_amount_total = fields.Boolean(
        string="Lock Amount Total",
        help="Prevent changes of amount total if SO approved by this user"
    )
...
class SaleOrderApprover(models.Model):
    ...
    _description = "SO Approver"
    _order = 'state asc'

    team_approver_id = fields.Many2one(
        comodel_name='sale.team.approver', string='SO Team Approver',
        ondelete='set null')
